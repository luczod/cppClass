// Section 20
// Iterators
// iterator.end() point to an index after the last one, i.e. null
#include <iostream>
#include <list>
#include <map>
// #include <set>
...
void test3()
{
    ...
    // auto it1 = nums1.cbegin(); // cbegin, is a const_interator
    ...
    it1 = nums1.begin();
    while(it1 != nums1.end()) {
        // *it1 = 0;   // Complier error - read only, is a const_interator
        it1++;
    }
}
...
void test4()
{
    ...
    auto it2 = name.crbegin(); // cosnt and reverser iterator over list of strings  point to Curly
    ...
    // iterator over a map
    std::map<std::string, std::string> favorites{ { "Frank", "C++" },
                                                  { "Bill", "Java" },
                                                  { "James", "Haskell" } };
