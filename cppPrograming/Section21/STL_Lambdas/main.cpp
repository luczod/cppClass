...
void test5()
{
    ...
    std::sort(people.begin(), people.end(),
              [](Person lhs, Person rhs) { return lhs.get_name() < rhs.get_name(); });
    ...
    std::sort(people.begin(), people.end(),
              [](Person lhs, Person rhs) { return lhs.get_age() > rhs.get_age(); });
...
bool in_between(const std::vector<int> &nums, int start_value, int end_value)
{
    ...
    // predicate lambda
    auto callback = [start_value, end_value](int value) {
        return value >= start_value && value <= end_value;
    };

    result = std::all_of(nums.begin(), nums.end(), callback);
...
void test6()
{
    ...
    std::iota(nums.begin(), nums.end(), 1); // nums(10) = 1,2,3...10
