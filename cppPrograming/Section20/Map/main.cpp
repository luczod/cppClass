...
void test1()
{
    ...
    auto it = m.find("Larry"); // return a pair
    if(it != m.end()) {
...
void test2()
{
    ...
    std::map<std::string, std::set<int>> grades{ { "Larry", { 100, 90 } },
                                                 { "Moe", { 94 } },
                                                 { "Curly", { 80, 90, 100 } } };

    display(grades);

    grades["Larry"].insert(95); // insert 95 into the std::set of grades for "Larry"

    display(grades);

    auto it = grades.find("Moe");
    // it->first is the key, it->second is the value (std::set)
    if(it != grades.end()) // found "Moe"
    {
        it->second.insert(1000); // insert 1000 into Moe's std::set of grades
    }
    display(grades);
}
