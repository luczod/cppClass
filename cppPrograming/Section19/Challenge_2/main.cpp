...
void print_footer(double average)
{
    std::cout << std::setw(20) << std::setfill('-') << "" << std::endl;
    std::cout << std::setfill(' '); // reset
    std::cout << std::setw(15) << std::left << "Average score" << std::setw(5) << std::right
              << average;
}
...
void print_student(const std::string &student, int score)
{
    ...
    std::cout << std::setw(15) << std::left << student << std::setw(5) << std::right << score
              << std::endl;
}
...
int main()
{
    ...
    in_file >> answer_key; // stores the first line of the file

    print_header();

    while(in_file >> name >> response) {
        // whithout a line
        // stores the first and second line of the file
        ++total_students;
    ...
    if(total_students != 0) {
        average_score = static_cast<double>(total_score) / total_students;
    }
