...
void test1()
{
    ...
    s1.insert(0); // add in order
    s1.insert(10);

    display(s1);

    if(s1.count(10)) { // return 0 or 1
...
void test2()
{
    ...
    it = stooges.find(Person("XXXX", 50)); // Will find James(50), uses operator "<"

    if(it != stooges.end()) {
        stooges.erase(it); // Will remove James,
    }
    display(stooges);
}
...
void test3()
{
    ...
    result = s.insert("A"); // return std::pair
    display(s);
